...
class RepresentationManager:
    ...
    async def _query_documents_recent(
        ...
        When session_allowlist is provided, only documents from those sessions
        (and only levels with a trustworthy session stamp) are returned — see
        ALLOWLIST_SAFE_LEVELS (session-purity invariant from upstream).
        """
        dedup_distance = settings.DERIVER.WORKING_REPRESENTATION_DEDUP_DISTANCE

        allowlist_levels = ", ".join(f"'{lvl}'" for lvl in ALLOWLIST_SAFE_LEVELS)
        session_filter = (
            f"AND d.session_name = ANY(:session_allowlist) "
            f"AND d.level IN ({allowlist_levels}) "
